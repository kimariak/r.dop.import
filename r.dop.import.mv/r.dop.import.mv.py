...
############################################################################
#
# MODULE:      r.dop.import.mv
# AUTHOR(S):   Johannes Halbauer, Anika Weinmann, Leon Louwarts
# PURPOSE:     Downloads DOPs for Mecklenburg-Vorpommern and AOI
...
# %module
# % description: Downloads DOPs for Mecklenburg-Vorpommern and AOI.
# % keyword: raster
# % keyword: import
# % keyword: DOP
...
TINDEX = (
    "https://github.com/mundialis/tile-indices/raw/main/DOP/MV/"
...
def main():
    """Main function of r.dop.import.mv"""
    aoi = options["aoi"]
    ...
    try:
        grass.message(
            _(f"Importing {number_tiles} DOPs for MV in parallel..."),
        )
        for tile in url_tiles:
            ...
            r_dop_import_worker_mv = Module(
                "r.dop.import.worker.mv",
                **param,
                run_=False,
            )
            # catch all GRASS output to stdout and stderr
            r_dop_import_worker_mv.stdout_ = grass.PIPE
            r_dop_import_worker_mv.stderr_ = grass.PIPE
            queue.put(r_dop_import_worker_mv)
        queue.wait()
    except Exception:
